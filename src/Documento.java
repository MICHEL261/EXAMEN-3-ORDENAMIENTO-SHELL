...
public class Documento {
    ...
    /*public static void metodoincersion(int inicio, int fin, int criterio){
        ...
        int i=0;
        int k=0;
        for (i = inicio+1; i < documentos.size();i++) {
            aux=documentos.get(i);

            
            for (k = i-1; k >=inicio && esMayor(documentos.get(k), aux, criterio);k--){
                documentos.set(k+1 , documentos.get(k));
                
                
            }

            documentos.set(k+1, aux);
    ...
    public static void metodoshell(int inicio, int fin, int criterio){
        int salto, i, j, k ;
        
        Documento aux;
        salto=documentos.size()/2;

        while(salto>0){
            for(i=salto; i<documentos.size();i++){
                j=i-salto;
                while(j>=0){
                   

                    k=j+salto;
                    if(esMayor(documentos.get(k), documentos.get(j), criterio)){
                        j=-1;

                    }else{
                        aux=documentos.get(j);
                        documentos.set(j, documentos.get(k));
                        documentos.set(k,aux);
